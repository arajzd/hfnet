...
if __name__ == '__main__':
    ...
    logging.info('Evaluating San Francisco with configuration: ')
    loc = Localization('medium4', args.model, config, build_db=args.build_db)

    query_file = 'queries_with_intrinsics.txt'
    queries, query_dataset = loc.init_queries(query_file, config_medium4)
    ...
    output_dir = Path(EXPER_PATH, 'eval/medium4')
    ...
    if args.export_poses:
        ...
        with open(poses_path, 'w') as f:
            for query, result in zip(queries, results):
                ...
                R = query_T_w[:-1][:, :-1]  # Upper left 3x3 sub-matrix
